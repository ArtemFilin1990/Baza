                ...
                for table in schema['tables']:
                    with self.subTest(table=table.get('name', 'unknown')):
                        ...
                        if 'uniqueKey' in table:
                            self.assertIsInstance(table['uniqueKey'], list, 
                                                "uniqueKey should be a list")
                        
                        if 'sort_by' in table:
                            self.assertIsInstance(table['sort_by'], list,
                                                "sort_by should be a list")
                ...
                for col_name, col_def in columns.items():
                    with self.subTest(schema=schema_file.name, 
                        ...
                        if isinstance(col_def, dict):
                            ...
                            self.assertIn('type', col_def, 
                                        f"Column {col_name} should have 'type'")
                            valid_types = ['string', 'number', 'integer', 'boolean']
                            self.assertIn(col_def['type'], valid_types,
                                        f"Column type should be one of {valid_types}")
                        else:
                            # Old format, just a type string
                            valid_types = ['string', 'number', 'integer', 'boolean']
                            self.assertIn(col_def, valid_types,
                                        f"Column type should be one of {valid_types}")
            ...
            with self.subTest(csv=csv_path):
                ...
                if csv_file.exists():
                    self.assertTrue(schema_file.exists(),
                                  f"CSV file {csv_path} should have schema {schema_path}")
                    ...
                    self.assertIn(csv_path, paths,
                                f"Schema {schema_path} should reference {csv_path}")
                    ...
                    for key in unique_keys:
                        with self.subTest(schema=schema_file.name,
                            ...
                            self.assertIn(key, columns,
                                        f"uniqueKey field '{key}' should be in columns")
                    ...
                    for field in sort_fields:
                        with self.subTest(schema=schema_file.name,
                            ...
                            self.assertIn(field, columns,
                                        f"sort_by field '{field}' should be in columns")
